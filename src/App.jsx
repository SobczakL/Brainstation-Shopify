...
function App() {
	...
	useEffect(() => {
		switch (stepCounter) {
			...
			case 2:
				...
				break;
			case 3:
				addMessage('Select a Style Descriptor to set the colour palette for your store', 'Bot');
				break;
			case 4:
				addMessage("Almost done! It's time for the final details!", 'Bot');
				break;
			case 5:
				addMessage(
					<div className='group-msg'>
						...
						<div>
							For additional support in editing your website you can visit the{' '}
							<span className='link'>Shopify Experts Marketplace</span>.
						</div>
						<div>We also offer 24/7 support for any issues that may arise.</div>
					</div>,
					'Bot'
				);
				break;
	...
	const handleStepChange = () => {
		if (stepCounter < 5) {
			...
			switch (stepCounter) {
				case 0:
					if (stepValues[stepCounter] === '') {
						addMessage("Warning! You didn't choose your business type!", 'Bot');
					} else {
						addMessage(
				...
				case 1:
					if (stepValues[stepCounter] === '') {
						addMessage("Warning! You didn't choose the location of your business!", 'Bot');
					} else {
						addMessage(`You\'ve selected ${stepValues[stepCounter]}. Terrific!`, 'Bot');
					}
					break;
				case 2:
					if (stepValues[stepCounter] === '') {
						addMessage("Warning! You didn't choose the your theme!", 'Bot');
					} else {
						addMessage(
				...
				case 3:
					if (stepValues[stepCounter] === '') {
						addMessage("Warning! You didn't choose the your color palette!", 'Bot');
					} else {
						addMessage(`You\'ve selected ${stepValues[stepCounter]}. Great choice!`, 'Bot');
					}
					break;
				case 4:
					if (stepValues[stepCounter] === '') {
						addMessage("Warning! You didn't upload the details!", 'Bot');
					} else {
	...
	const handleContinueClick = () => {
		addMessage("We'll start with basic information about your business", 'Bot');
	};
	...
	const handleStepValueChange = (updatedValue) => {
		...
		if (stepValues[stepCounter] === '') {
			...
			setmessages([...newMessages, { message: updatedValue, sender: 'User' }]);
		}
	};
	...
	const [messages, setmessages] = useState([
		...
		{
			message: "You'll see options displayed to the left of this chat window",
			sender: 'Bot',
		},
	]);
		...
		if (sender === 'User') {
			getFeedback(text)
				.then((response) => {
					...
					if (keyWord[0] === 'user.step') setStepCounter(Number(keyWord[1]) - 1);
				})
				.catch((_error) => {
	...
	return (
		<>
			...
			<BrowserRouter>
				<Routes>
					<Route path='/' element={<LandingPage handleContinueClick={handleContinueClick} />} />
					<Route
						path='/process-page'
						element={
							<ProcessPage
